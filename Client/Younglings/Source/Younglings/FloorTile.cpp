	...
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex,bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if(RunCharacter)
	{
		...
		GetWorld()->GetTimerManager().SetTimer(DestroyHandle, this, &AFloorTile::DestroyFloorTile, 2.f, false);
	}
}
...
void AFloorTile::BeginPlay()
{
	...
	check(RunGameMode); //check if GameMode is valid
...
void AFloorTile::SpawnLaneItem(UArrowComponent* Lane, int32& NumBigs)
{
	...
	else if(UKismetMathLibrary::InRange_FloatFloat(RandVal, SpawnPercent2+0.1f, SpawnPercent3, true, true))
	{
		if (NumBigs <= 1)
		{
	...
	else if(UKismetMathLibrary::InRange_FloatFloat(RandVal, SpawnPercent3+ 0.1f, 0.9f, true, true))
	{
